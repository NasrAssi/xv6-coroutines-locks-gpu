...
// ── map_display support ───────────────────────────────────────────────
// Return the physical address of framebuffer page i (0 <= i < FB_PAGES),
// or 0 if i is out of range.  Kernel memory is direct-mapped, so the
// kalloc'd address stored in fb[i] is already a physical address.
uint64
...
// ── flip_display support ──────────────────────────────────────────────
// pid of the process whose buffer the device is currently scanning out,
// or -1 when the device is backed by the kernel framebuffer fb[].
static int flip_owner_pid = -1;
// Physical pages the device was last flipped to, so that when the owning
// process exits we can copy its final frame into the kernel framebuffer.
static uint64 flip_pas[FB_PAGES];

// Re-attach the kernel's own framebuffer pages as the device backing.
static void
gpu_attach_kernel_fb(void)
{
...
// Zero-copy page flip.  Re-point the display resource's backing list at the
// physical pages in pas[0..n) (the calling process's back buffer).  No pixel
// data is copied — only the backing entries change.  pid records the owner so
// the kernel can restore its own framebuffer if that process exits while the
// device is still reading its pages.  Returns 0 on success, -1 on error.
int
virtio_gpu_flip(uint64 *pas, int n, int pid)
{
    if (n != FB_PAGES)
        return -1;
...
// Called when a process exits.  If it owned the current flip backing, the
// device is still pointing at pages that are about to be freed.  Copy the
// process's final frame into the kernel framebuffer (so the last image
// stays on screen) and re-attach fb[] as the backing before those pages
// disappear.
void
